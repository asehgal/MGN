...
-(void)_updateSession
{
    if (!appDelegate.session.isOpen)
    {
        ...
        if (appDelegate.session.state == FBSessionStateCreatedTokenLoaded ||
            appDelegate.session.state == FBSessionStateCreated||appDelegate.session.state ==
            FBSessionStateClosedLoginFailed||
            appDelegate.session.state ==
...
{
    NSLog(@"%d",appDelegate.session.state);
         ...
         if( [_savedUserId length] < 1 || [_savedUserId isEqualToString:@"(null)"] || !_savedUserId )
         {
             ...
             if (error) {
                 ...
                 if( _completion )
                 {
                     ...
                     [FBSession setActiveSession:nil];                     return ;
                 }
             ...
             [_fbRequest startWithCompletionHandler:^(FBRequestConnection *connection, id result, NSError *error)
              {
                  ...
                  if( [result isKindOfClass:[NSDictionary class]] )
                  {
                      ...
                      if( [_userInfo count] > 0 )
                      {
                          ...
                          NSString *_accessToken = session.accessToken;
...
-(NSDictionary *)_getUploadParamsWithVideoLocalPath:(NSString *)_videoPath title:(NSString *)_videoTitle description:(NSString *)_videoDescription
{
    ...
    if( _videoPath )
    {
        _params = [self _getUploadParamsWithVideoData:[NSData dataWithContentsOfURL:[NSURL URLWithString:_videoPath]]
                                             videoURL:_videoPath
                                                title:_videoTitle
                                          description:_videoDescription];
    }
